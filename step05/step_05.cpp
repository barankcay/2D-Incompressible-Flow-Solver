...
int main()
{
    ...
    {
        U_outputFileColoc<< std::fixed << std::setprecision(7) << (j - 1) * h + h / 2 << "    "
                     << std::fixed << std::setprecision(7) << 0.5 * (u[(Nx / 2) - 1][j] + u[(Nx / 2)][j]) << "\n";
    }
...
void calculateGhostCellValues(int b, vector<vector<double>> &M,double pressureOutlet, double uTopWall, double uBottomWall, double uLeftWall, double uRightWall, double vLeftWall, double vRightWall, double vTopWall, double vBottomWall, int Nx, int Ny)
{
    ...
    if (b == 0)
    {
        for (int j = Ny - 2; j >= 1; j--)
        {
            ...
            M[Nx - 1][j] = M[Nx - 2][j]; // Right wall, dp/dx = 0
        }
    ...
    else if (b == 1)
    {
        for (int j = Ny - 2; j >= 1; j--)
        {
            M[0][j] = 2 * uLeftWall - M[1][j]; // Left wall,ghost cell,  u = 0

            M[Nx - 1][j] = 2 * uRightWall - M[Nx - 2][j]; // Right wall, u = 0
        }
        for (int i = Nx - 2; i >= 1; i--)
        {
            M[i][0] = 2 * uBottomWall - M[i][1];                      // Top wall, ghost cell
            M[i][Ny - 1] = 2 * uTopWall - M[i][Ny - 2]; // Bottom wall, ghost cell
        }
    }
    else if (b == 2)
    {
        ...
        for (int i = Nx - 2; i >= 1; i--)
        {
            M[i][0] = 2 * vBottomWall - M[i][1];                      // Top wall, v = 0
            M[i][Ny - 1] = 2 *vTopWall  - M[i][Ny - 2]; // Bottom wall, v = 0
        }
        for (int j = Ny - 2; j >= 1; j--)
        {
            M[0][j] = 2 * vLeftWall - M[1][j];                    // Left wall, ghost cell
            M[Nx - 1][j] = 2 * vRightWall - M[Nx - 2][j]; // Right wall, ghost cell
        }
    }
