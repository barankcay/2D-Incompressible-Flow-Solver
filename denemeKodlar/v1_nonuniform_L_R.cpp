...
struct solverConfig
{
    ...
    double Cf;                      // friction coefficient for the non-uniform mesh
    double wallShearStress;         // wall shear stress for the non-uniform mesh
    double utau;                    // friction velocity for the non-uniform mesh
    double firstCellHeight;         // height of the first cell for the non-uniform mesh
    double inflationLayerThickness; // boundary layer thickness for the non-uniform mesh
    double growthRate;              // growth rate for the non-uniform mesh
    double yPlus;
    int numberOfMeshLayers; // number of mesh layers for the non-uniform mesh
};
...
void nonUniformMesh(solverConfig &cfg)
{
    ...
    cfg.inflationLayerThickness = cfg.firstCellHeight;
    for (int i = 0; i < cfg.numberOfMeshLayers - 1; i++)
    {
        cfg.inflationLayerThickness = cfg.firstCellHeight + cfg.inflationLayerThickness * cfg.growthRate; // boundary layer thickness
    }
    if (2 * cfg.inflationLayerThickness > cfg.lengthY)
    {
        cout << "Inflation layer thickness is " << cfg.inflationLayerThickness << endl;
        cout << "Inflation layer thickness is greater than the length of the domain in the y-direction" << endl;
        cout << "Please reduce the number of mesh layers or the growth rate" << endl;
        exit(1);
    }
    cfg.inflationLayerThickness = 2 * cfg.inflationLayerThickness; // double the layer because of left and right wall

    double largestBLmesh = cfg.firstCellHeight * pow(cfg.growthRate, cfg.numberOfMeshLayers - 1);
    cfg.Nx = 2 + 2 * cfg.numberOfMeshLayers + ((cfg.lengthX - cfg.inflationLayerThickness) / (largestBLmesh * cfg.growthRate)); // Number of cells in the y-direction, including ghost cells
...
{
    f.ux[0] = 0;
    f.vx[0] = cfg.firstCellHeight / 2;
    f.vx[cfg.Nx - 3] = cfg.lengthX - cfg.firstCellHeight / 2;
    f.ux[cfg.Nx - 3] = cfg.lengthX - cfg.firstCellHeight;
    for (int i = 1; i < cfg.numberOfMeshLayers; i++)
    {
        f.ux[i] = cfg.firstCellHeight + (f.ux[i - 1]) * cfg.growthRate;                                                     // x coordinate of u velocity field
        f.vx[i] = cfg.firstCellHeight + (f.vx[i - 1]) * cfg.growthRate;                                                     // y coordinate of u velocity field
        f.ux[cfg.Nx - 3 - i] = cfg.lengthX - (cfg.firstCellHeight + (cfg.lengthX - f.ux[cfg.Nx - 2 - i]) * cfg.growthRate); // x coordinate of u velocity field
        f.vx[cfg.Nx - 3 - i] = cfg.lengthX - (cfg.firstCellHeight + (cfg.lengthX - f.vx[cfg.Nx - 2 - i]) * cfg.growthRate); // y coordinate of u velocity field
    }
    cfg.hx = (cfg.lengthX - cfg.inflationLayerThickness) / (cfg.Nx - 2 - 2 * cfg.numberOfMeshLayers); // Grid spacing in the y-direction
    ...
    {
        for (int i = cfg.numberOfMeshLayers; i < cfg.Nx - 2 - cfg.numberOfMeshLayers; i++)
        {
            ...
            f.vy[j] = (cfg.Ny - j - 3) * cfg.hy; // y coordinate of v velocity field
        }
    }
...
void updateTimeStepSize(fields &f, solverConfig &cfg)
{
    ...
    for (int i = 1; i < cfg.Nx - 1; i++)
    {
        for (int j = 1; j < cfg.Ny - 1; j++)
        {
            double rule1 = (pow(cfg.hx, 2) * pow(min(cfg.firstCellHeight, cfg.hy), 2)) / (2 * cfg.dynamicViscosity * (pow(cfg.hx, 2) + pow(min(cfg.firstCellHeight, cfg.hy), 2)));
...
void writeFinalVTKFile(const fields &f, const solverConfig &cfg)
{
...
int main()
{
    ...
    cfg.timeTolerance = 1e-7;            // Tolerance for the time loop convergence
    ...
    cfg.firstCellHeight = 0.01;          // y+ value for the first cell height

    cfg.numberOfMeshLayers = 10; // Number of mesh layers for the non-uniform mesh
    cfg.growthRate = 1.1;        // Growth rate for the non-uniform mesh
    ...
    cfg.timeStepSize = min((pow(cfg.hx, 2) * pow(min(cfg.firstCellHeight, cfg.hy), 2)) / (2 * cfg.dynamicViscosity * (pow(cfg.hx, 2) + pow(min(cfg.firstCellHeight, cfg.hy), 2))), 2 * cfg.dynamicViscosity / (pow(cfg.uTopWall, 2) + pow(cfg.vLeftWall, 2)));
