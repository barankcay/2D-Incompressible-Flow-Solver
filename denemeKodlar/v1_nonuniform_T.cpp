...
{
    if (cfg.numberOfMeshLayers > cfg.Ny - 2)
    {
        cout << "Number of mesh layers is greater than the number of cells in the y-direction. Please reduce the number of mesh layers." << endl;
        exit(1);
    }
    double geomSeries = ((1 - pow(cfg.growthRate, cfg.numberOfMeshLayers)) / (1 - cfg.growthRate));

    if (cfg.numberOfMeshLayers == 0)
    {
        geomSeries = 0; // height of the first cell for the non-uniform mesh
    }
    double G = geomSeries + (cfg.Ny - 2 - cfg.numberOfMeshLayers) * pow(cfg.growthRate, cfg.numberOfMeshLayers);
    cfg.firstCellHeight = cfg.lengthY / G; // height of the first cell for the non-uniform mesh

    cfg.inflationLayerThickness = 0;
    for (int i = 0; i < cfg.numberOfMeshLayers; i++)
    {
        cfg.inflationLayerThickness = cfg.firstCellHeight + cfg.inflationLayerThickness * cfg.growthRate; // boundary layer thickness
    }

    if (cfg.inflationLayerThickness > cfg.lengthY)
    {
        cout << "Inflation layer thickness is greater than the length of the domain in the y-direction. Please reduce the number of mesh layers or the growth rate." << endl;
        exit(1);
    }
}
...
{
    f.uy[0] = cfg.lengthY - cfg.firstCellHeight / 2;
    f.vy[0] = cfg.lengthY - cfg.firstCellHeight;
    for (int j = 1; j < cfg.numberOfMeshLayers; j++)
    {
        f.uy[j] = cfg.lengthY - (cfg.firstCellHeight + (cfg.lengthY - f.uy[j - 1]) * cfg.growthRate);
        f.vy[j] = cfg.lengthY - (cfg.firstCellHeight + (cfg.lengthY - f.vy[j - 1]) * cfg.growthRate); // y coordinate of u velocity field
    }
    cfg.hy = (cfg.lengthY - cfg.inflationLayerThickness) / (cfg.Ny - 2 - cfg.numberOfMeshLayers); // Grid spacing in the y-direction
    // cfg.hy = cfg.firstCellHeight * pow(cfg.growthRate, cfg.numberOfMeshLayers);
    cout << "Grid spacing in the y-direction: " << cfg.hy << endl;
    for (int j = cfg.numberOfMeshLayers + 1; j < cfg.Ny - 2; j++)
    {
...
int main()
{
    ...
    cfg.Nx = 22;                         // Number of cells in the x-direction, including ghost cells
    cfg.Ny = 22;                         // Number of cells in the y-direction, including ghost cells
    ...
    cfg.numberOfMeshLayers = 0; // Number of mesh layers for the non-uniform mesh
    cfg.growthRate = 1.2;       // Growth rate for the non-uniform mesh
    ...
    cout << endl;
    cout << "First cell height is " << cfg.firstCellHeight << endl;
