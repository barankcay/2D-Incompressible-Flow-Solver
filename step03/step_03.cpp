...
int main()
{
    ...
    for (int i = 1; i < Nx - 1; i++)
    {
        U_outputFileStag << std::fixed << std::setprecision(7) <<  (i - 1) * h << "    "
    ...
    for (int i = 1; i < Nx - 1; i++)
    {
        P_outputFileStag << std::fixed << std::setprecision(7) <<  (i - 1) * h + h / 2 << "    "
...
void calculateGhostCellValues(int b, vector<vector<double>> &M,double pressureOutlet, double uTopWall, double uBottomWall, double uLeftWall, double uRightWall, double vLeftWall, double vRightWall, double vTopWall, double vBottomWall, int Nx, int Ny)
{
    if (b == 0)
    {
        ...
        for (int j = 1; j < Ny-1; j++)
        {
            ...
            M[Nx - 1][j] = 2*pressureOutlet -M[Nx-2][j]; // Right wall, dp/dy = 0
        }
    ...
    else if (b == 1)
    {
        ...
        for (int j = 1; j < Ny-1; j++)
        {
            M[0][j] = 2 * uLeftWall - M[2][j];              // Top wall, ghost cell
            M[1][j]=uLeftWall;
            M[Nx-1][j]=M[Nx-2][j];
